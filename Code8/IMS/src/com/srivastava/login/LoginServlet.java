...
public class LoginServlet extends HttpServlet {
	...
	private String doDesign(String color, boolean ...flag){
		...
		String script = "<script>if(!navigator.cookieEnabled){"
		 ...
		 design = "<!DOCTYPE html><html lang='en'><head>"
				...
				+"<input type='hidden' name='branch' value='Noida'/>"
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(userid.equals(password)){
			...
			RequestDispatcher rd = request.getRequestDispatcher("welcome.onlineshop");
			
			rd.forward(request, response);
			//String urlWithSessionId = response.encodeRedirectURL("welcome?uname="+userid+"&branch="+branchName);
